...
const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY || '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';
...
const MODEL = MODEL_SONNET; // default for health check display
...
function selectModel(question, hasPhotos, historyLength) {
  ...
  if (hasPhotos) return { model: MODEL_SONNET, reason: 'photo-analysis' };

  // 2. Long conversations (6+ messages) — needs better context tracking
  if (historyLength >= 6) return { model: MODEL_SONNET, reason: 'long-conversation' };
  ...
  if (safetyCritical.some(term => q.includes(term))) {
    return { model: MODEL_SONNET, reason: 'safety-critical' };
  }
  ...
  if (complexPatterns.some(pat => pat.test(q))) {
    return { model: MODEL_SONNET, reason: 'complex-diagnostic' };
  }

  // Everything else → Haiku (simple how-tos, basic info, quick tips)
  return { model: MODEL_HAIKU, reason: 'simple-query' };
}
...
app.get('/api/health', (req, res) => {
  ...
  res.json({
    ...
    routing: 'two-tier (haiku for simple, sonnet for complex/photos/safety)',
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    // ─── MODEL SELECTION: Route to Haiku or Sonnet ──────────────────
    const modelChoice = selectModel(question, hasPhotos, history.length);
    const selectedModel = modelChoice.model;

    console.log(`[${new Date().toISOString()}] Sending ${messages.length} messages (${history.length} history + 1 new) → ${selectedModel} (${modelChoice.reason})`);
